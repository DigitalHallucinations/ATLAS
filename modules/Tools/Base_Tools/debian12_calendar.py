...
class _CalendarWriteHelpers:
    """Shared helpers for backends that support write operations."""
...
class DBusCalendarBackend(_CalendarWriteHelpers, CalendarBackend):
    """Backend that communicates with the Debian 12 DBus calendar service."""

    def __init__(self, client: Any, default_timezone: ZoneInfo) -> None:
        self._client = client
    ...
    async def list_events(
        ...
        try:
            ...
            raise CalendarBackendError("Failed to list Debian 12 DBus calendar events") from exc
    ...
    async def get_event(
        ...
        try:
            ...
            raise CalendarBackendError("Failed to fetch Debian 12 DBus calendar event") from exc
    ...
    async def search_events(
        ...
        try:
            ...
            raise CalendarBackendError("Failed to search Debian 12 DBus calendar events") from exc
    ...
    async def create_event(
        ...
        try:
            response = await self._client.create_event(request_payload, calendar=calendar)
        except Exception as exc:  # pragma: no cover - defensive fallback
            raise CalendarBackendError("Failed to create Debian 12 DBus calendar event") from exc

        if isinstance(response, Mapping):
            normalized_response = self._coerce_backend_payload(response, calendar)
            calendar_name = normalized_response.get("calendar") or self._default_calendar(calendar)
    ...
    async def update_event(
        ...
        try:
            ...
            if self._is_event_not_found(exc):
                raise EventNotFoundError(event_id) from exc
            raise CalendarBackendError("Failed to fetch Debian 12 DBus calendar event for update") from exc
        ...
        try:
            response = await self._client.update_event(event_id, request_payload, calendar=calendar)
        except EventNotFoundError:
            raise
        except Exception as exc:  # pragma: no cover - defensive fallback
            if self._is_event_not_found(exc):
                raise EventNotFoundError(event_id) from exc
            raise CalendarBackendError("Failed to update Debian 12 DBus calendar event") from exc
        ...
        calendar_name = current_payload.get("calendar") or self._default_calendar(calendar)
    ...
    async def delete_event(
        ...
        try:
            ...
            if self._is_event_not_found(exc):
                raise EventNotFoundError(event_id) from exc
            raise CalendarBackendError("Failed to delete Debian 12 DBus calendar event") from exc

    def _coerce_backend_payload(
        self, payload: Mapping[str, Any], calendar_hint: Optional[str]
    ) -> Dict[str, Any]:
        event_id = self._coerce_optional_str(
            payload.get("id") or payload.get("uid") or payload.get("event_id")
        )
        if not event_id:
            raise CalendarBackendError("DBus calendar payload is missing an identifier")
    ...
    def _normalize_backend_attendees(self, value: Any) -> List[Dict[str, Optional[str]]]:
        if value is None:
            return []
    ...
    def _default_calendar(calendar_hint: Optional[str]) -> str:
        if calendar_hint and str(calendar_hint).strip():
            return str(calendar_hint)
        return "primary"


class ICSCalendarBackend(_CalendarWriteHelpers, CalendarBackend):
...
class Debian12CalendarTool:
    ...
    def _build_backend(self) -> CalendarBackend:
        ...
        if backend_kind == "dbus":
            client = self._resolve_dbus_client()
            if client is None:
                logger.warning(
                    "Debian 12 calendar DBus backend requested but no client is configured"
                )
                return NullCalendarBackend()
            return DBusCalendarBackend(client, timezone)
    ...
    def _resolve_dbus_client(self) -> Optional[Any]:
        existing = getattr(self, "_dbus_client", None)
        if existing is not None:
            return existing

        factory_spec = self._get_config("DEBIAN12_CALENDAR_DBUS_FACTORY")
        ...
        if ":" in spec:
            ...
            parts = spec.rsplit(".", 1)
            if len(parts) != 2:
                logger.error("Invalid DBus client factory path '%s'", spec)
                return None
        ...
        try:
            ...
            logger.error(
                "Failed to construct Debian 12 DBus calendar client from '%s'",
                spec,
                exc_info=exc,
            )
            return None

        self._dbus_client = client
        return client
