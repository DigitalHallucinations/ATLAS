...
source_of_truth: modules/Server/dlp.py; modules/Server/routes.py; server/http_gateway.py
---
...
- Residency and DLP enforcement apply to requests that traverse `AtlasServer`
  (HTTP gateway or in-process callers). Payloads are sanitized via
  `DataLossPreventionEnforcer` before repository calls and when importing backup
  bundles.【F:modules/Server/routes.py†L1287-L1325】【F:modules/Server/routes.py†L3143-L3226】
- Direct database access or bespoke scripts that bypass `AtlasServer` (for
  example, ad hoc SQL imports) are out of scope and must be handled with
  external controls.
- Export controls do not automatically scrub assets generated by third-party
  providers; keep those integrations behind the same HTTP routes or add
  provider-side filters to match the documented patterns.【F:modules/Server/dlp.py†L1-L78】
