...
- **Echo** – Invite Echo to facilitate tense conversations. The persona blends `tone_analyzer`, `reflective_prompt`, `memory_recall`, and `conflict_resolver` to surface emotional cues, replay prior commitments, and co-create next steps. Echo keeps `context_tracker` memories short-lived and timestamps agreements with `get_current_info` so consent and accountability remain explicit.
