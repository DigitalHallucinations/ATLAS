...
- **Hermes** – Route complex ingestion handoffs to Hermes when multiple connectors must be sequenced. The persona choreographs `api_connector`, `file_ingest`, `stream_monitor`, `schema_infer`, and `data_bridge`, documenting governance checkpoints and publishing dry-run results so stewards can approve production cuts. Capture source contracts, refresh cadences, and observability requirements in task metadata to receive a compliant playbook.
- **Specter** – Engage Specter when you need a calm security analyst to interrogate telemetry. The persona leans on `log_parser`, `threat_scanner`, `sys_snapshot`, `audit_reporter`, and `incident_summarizer` to sift noisy alerts into actionable investigations, pairing every escalation with policy-aligned guidance from `policy_reference`.
- **Muse** – Pair with Muse when you need narrative scaffolds, lyric drafts, or visual ideation. The persona blends `storyweaver`, `lyricist`, `visual_prompt`, `mood_map`, and `emotive_tagger` so every creative brief arrives with tonal guidance, emotional checkpoints, and follow-up prompts. Use `context_tracker` to pin reference beats before switching mediums.
- **Echo** – Bring Echo into conversations where tone needs moderating and commitments must be documented. Echo relies on `tone_analyzer`, `reflective_prompt`, `memory_recall`, and `conflict_resolver` (anchored by `context_tracker` and `get_current_info`) to mirror emotions, invite reflection, surface prior agreements, and shape next steps.
