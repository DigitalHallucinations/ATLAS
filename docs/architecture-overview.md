...
## Runtime Composition
- **Messaging**: `ConfigManager.configure_message_bus()` wires Redis streams or in-memory queues depending on the environment. Tool events can still flow through the legacy `modules/Tools/tool_event_system` adapters, but those adapters are **not** auto-bridged to the message bus—you must connect them manually if you need both channels to interoperate.
